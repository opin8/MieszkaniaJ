...
function AddApartment() {
  ...
  const [form, setForm] = useState({
    ...
    storageUnit: false,
    parkingSpotNumber: "",
    balconyTerraceArea: "",   // NOWE POLE
    garageNumber: "",         // NOWE POLE
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post("/apartments", {
        ...
        balconyTerraceArea: form.balconyTerraceArea ? Number(form.balconyTerraceArea) : null,  // NOWE
        garageNumber: form.garageNumber || null,  // NOWE
      });
      navigate("/dashboard");
    } catch (err) {
    ...
    <div className="add-container">
      ...
      <div className="add-card">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input
            placeholder="Numer miejsca parkingowego (opcjonalne)"
            type="number"
            value={form.parkingSpotNumber}
            onChange={e => setForm({ ...form, parkingSpotNumber: e.target.value })}
          />
          {/* NOWE POLA */}
          <input
            placeholder="Metraż balkonu/tarasu (m²)"
            type="number"
            step="0.1"
            value={form.balconyTerraceArea}
            onChange={e => setForm({ ...form, balconyTerraceArea: e.target.value })}
          />
          <input
            placeholder="Numer garażu"
            value={form.garageNumber}
            onChange={e => setForm({ ...form, garageNumber: e.target.value })}
          />
